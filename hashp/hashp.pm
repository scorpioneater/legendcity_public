...
sub Container::new {  my $class = shift;
    ...
    bless $self,$class;   
return $self;};
sub Container::toString {  my ($self) = @_;  
my $name = $self->{_name};  
my $symbol = $self->{_symbol};  
my $ret = "name:${name} symbol:${symbol} ";  
keys %{$self->{_body}};  
while(my ($k,$v) = each %{$self->{_body}}){    
     $ret .= "key:${k} values:";    
     map {$ret .= "${_},"} @{$v};    
     $ret .= " ";  }  
     return $ret;};
sub Container::addArgsToBody{   
     my ($self,$key,$arr) = @_;  
     $self->{_body}{$key} = $arr;
};
sub Container::get{  
